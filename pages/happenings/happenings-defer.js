...
const handleHappenings = (happenings) => {
    ...
    const renderEventCard = (event) => {
        return `<section class="bg-ams-white xmed:shadow-2xl mb-4 border-b xmed: border-0 w-96 h-96">
            <figure>
                <figcaption>
                  <img src="${event.image}" alt="${event.title}" loading="lazy" class="w-full" decoding="async" alt="">
            ...
            <div class="pb-5 text-left pl-6 bg-ams-white text-xs text-ams-primary font-medium"> <a  href="/happenings-detail/?id=${
                event.id
            }" class="inline-block py-2 px-8 bg-ams-gold text-ams-white font-serif tracking-wide button--primary button"> View More </a></div>
          </section>`;
        };
    ...
    var Happenings = function (options) {
        ...
        const our_happenings_title = document.querySelector(
            "#our_happenings_title"
        );
        const screen_width =
            window.innerWidth ||
        ...
        self._onClick = function (e) {
            ...
            if (screen_width < 900) {
                if (hasClass(target, "calendar-date")) {
                    ...
                    if (evList.length) {
                        our_happenings_title.innerHTML =
                            "Happenings for <br/>" +
                            months[self.calendar.month] +
                            " " +
                            day;
        ...
        self._mouseover = function (e) {
            ...
            if (hasClass(target, "calendar-date")) {
                ...
                hasClass(target, "c-event") ||
                hasClass(target, "d-event")
            ) {
                const parentEle = target.parentElement;
                let id;
                ...
                const day = Number(
                    parentEle.querySelector(".calendar-date").innerHTML
                );
